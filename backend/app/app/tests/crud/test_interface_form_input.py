...
def test_create_form_input(db: Session, superuser: User) -> None:
    ...
    form_input = form_input_crud.create(db, obj_in=form_input_create)
    assert form_input
    assert form_input.id
    assert form_input.name == name
    assert form_input.date_created == date_created
    assert form_input.an_integer == an_integer
...
def test_get_form_input(db: Session, superuser: User) -> None:
    ...
    assert stored_form_input.id == form_input.id
    assert stored_form_input.name == form_input.name
    assert stored_form_input.date_created == form_input.date_created
    assert stored_form_input.an_integer == form_input.an_integer
...
def test_get_multi_form_input(db: Session, superuser: User) -> None:
    ...
    form_input_crud = crud.interface.get_interface_crud(db, id=interface.id)
    for form_input_in in form_inputs_in:
        form_input_crud.create(db, obj_in=form_input_in)
    stored_form_inputs = form_input_crud.get_multi(db=db)
    for fii in form_inputs_in:
        found_match = False
        for sfi in stored_form_inputs:
            name_match = fii["name"] == sfi.name
            date_match = fii["date_created"] == sfi.date_created
            integer_match = fii["an_integer"] == sfi.an_integer
...
def test_update_form_input(db: Session, superuser: User) -> None:
    ...
    form_input2 = form_input_crud.update(
        db=db, db_obj=form_input, obj_in=form_input_update
    )
    assert form_input2
...
def test_delete_form_input(db: Session, superuser: User) -> None:
    ...
    form_input_crud = crud.interface.get_interface_crud(db, id=interface.id)
    form_input = form_input_crud.create(db, obj_in=form_input_create)
    form_input2 = form_input_crud.remove(db, id=form_input.id)
    form_input3 = form_input_crud.get(db=db, id=form_input.id)
