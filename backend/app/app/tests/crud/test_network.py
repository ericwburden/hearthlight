...
def test_create_network(db: Session, normal_user: User) -> None:
    ...
    network = crud.network.create(
        db=db, obj_in=network_in, created_by_id=normal_user.id
    )
...
def test_get_multi_network(db: Session, normal_user: User) -> None:
    ...
    new_networks = [
        crud.network.create(db=db, obj_in=network_in, created_by_id=normal_user.id)
        for network_in in new_networks_in
    ]
...
def test_update_network(db: Session, normal_user: User) -> None:
    ...
    network2 = crud.network.update(
        db=db, db_obj=network, obj_in=network_update, updated_by_id=normal_user.id
    )
