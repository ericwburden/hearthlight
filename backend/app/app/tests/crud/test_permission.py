...
from app.schemas.permission import PermissionCreate, PermissionUpdate, PermissionTypeEnum, ResourceTypeEnum
...
def test_create_node_permission(db: Session, normal_user: User) -> None:
    node = create_random_node(db, created_by_id=normal_user.id, node_type='test_create_permission')
    permission_type = random.choice(list(PermissionTypeEnum))
    permission_in = PermissionCreate(resource_id=node.id, resource_type=ResourceTypeEnum.node, permission_type=permission_type)
...
def test_get_node_permission(db: Session, normal_user: User) -> None:
    node = create_random_node(db, created_by_id=normal_user.id, node_type='test_get_permission')
    permission_type = random.choice(list(PermissionTypeEnum))
    permission_in = PermissionCreate(resource_id=node.id, resource_type=ResourceTypeEnum.node, permission_type=permission_type)
    permission = crud.node_permission.create(db=db, obj_in=permission_in)
...
def test_get_multi_node_permission(db: Session, normal_user: User) -> None:
    node = create_random_node(db, created_by_id=normal_user.id, node_type='test_get_multi_permission')
    permission_types = [random.choice(list(PermissionTypeEnum)) for n in range(10)]
    new_permissions_in = [PermissionCreate(resource_id=node.id, resource_type=ResourceTypeEnum.node, permission_type=pt) for pt in permission_types]
    new_permissions = [
    ...
    stored_permission_permission_types = [sp.permission_type for sp in stored_permissions]
...
def test_update_node_permission(db: Session, normal_user: User) -> None:
    node1 = create_random_node(db, created_by_id=normal_user.id, node_type='test_update_permission')
    permission_type1 = random.choice(list(PermissionTypeEnum))
    permission_in = PermissionCreate(resource_id=node1.id, resource_type=ResourceTypeEnum.node, permission_type=permission_type1)
    permission = crud.node_permission.create(db=db, obj_in=permission_in)
    
    node2 = create_random_node(db, created_by_id=normal_user.id, node_type='test_update_permission')
    ...
    permission_update = PermissionUpdate(resource_id=node2.id, permission_type=permission_type2)
...
def test_delete_node_permission(db: Session, normal_user: User) -> None:
    node = create_random_node(db, created_by_id=normal_user.id, node_type='test_delete_permission')
    permission_type = random.choice(list(PermissionTypeEnum))
    permission_in = PermissionCreate(resource_id=node.id, resource_type=ResourceTypeEnum.node, permission_type=permission_type)
    permission = crud.node_permission.create(db=db, obj_in=permission_in)
