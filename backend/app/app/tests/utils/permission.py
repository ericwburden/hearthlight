...
def create_random_permission(
    ...
    *,
    ...
    if node_id is None:
        node = create_random_node(db, created_by_id=created_by_id, node_type='random_node')
        node_id = node.id
    permission_type = random.choice(list(PermissionTypeEnum))
    permission_in = PermissionCreate(resource_id=node_id, resource_type=ResourceTypeEnum.node, permission_type=permission_type)
